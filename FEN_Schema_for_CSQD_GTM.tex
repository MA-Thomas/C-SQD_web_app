...
The \textbf{audit subject} --- a scoped claim, claim-warrant bundle,
decision-relevant assertion, or artifact-attached claim under evaluation --- is
not an ontological primitive. It is the \texttt{subject\_id} referent on every
\texttt{Fact}: the thing the facts are \emph{about}. Papers, models, clinical
trial protocols, datasets, benchmarks, and other artifacts may be attached as
evidence artifacts and discovery surfaces without becoming the audit's
epistemic target. This is analogous to the patient in clinical FEN. The same
audit subject may be referenced by multiple independent \texttt{AuditEpisode}s
commissioned by different sponsors at different times.
...
An \texttt{AuditSubject} is the scoped epistemic target under evaluation: a
claim, claim-warrant bundle, decision-relevant assertion, or artifact-attached
claim. It is \emph{not} an ontological primitive in the FEN sense. It is the
referent of \texttt{subject\_id} on every \texttt{Fact} --- the thing the facts
are about. The subject type enum includes common artifact anchors because a
claim may be attached to a manuscript, preprint, dataset, code repository,
clinical trial protocol, AI model evaluation, benchmark, policy document, grant
proposal, or technical report; these artifacts are evidence and discovery
surfaces, not automatically verified evidence tokens. Claim-scoped audit
subjects can be represented with \texttt{Other(String)} until claim-specific
variants are promoted into the enum. The design rationale is discussed in
Section~\ref{sec:subject-rationale}.
...
\begin{itemize}
  ...
  \item $S$: stakes of the target claim or episode-scoped audit question, operationalized per domain instantiation
  \item $L$: scrutiny depth, weighting \texttt{ElementReview} facts by reviewer expertise and solicitation status
  \item $U$: uptake of the target claim and its audit record, operationalized per domain instantiation
\end{itemize}
...
The audit subject --- the scoped claim, claim-warrant bundle, or
artifact-attached claim --- is the thing the facts are \emph{about}. It is not
itself a fact, episode, or narrative. In clinical FEN, the patient occupies the
same structural position: every \texttt{Fact} carries a \texttt{subject\_id},
but the patient is not in the FEN ontology.

This separation has a concrete consequence: the same claim can be the subject of
multiple independent \texttt{AuditEpisode}s commissioned by different sponsors
at different times. Each episode has its own facts, its own memberships, and
its own synthesis reviews. They do not collapse into a single entity merely
because they share a subject. A venture capital firm evaluating an academic
finding for a licensing decision and an independent journal seeking external
review of the same artifact-attached claim produce entirely separate audit
records. A paper can still be searched, cited, and opened as a discovery
surface showing every audit that uses it as evidence, challenges it, or audits a
claim made within it.
...
Subject &
  \texttt{AuditSubject} &
  Referenced audit-target metadata \\
Users &
